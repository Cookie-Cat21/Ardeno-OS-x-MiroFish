...
Uses LLM to automatically generate detailed simulation parameters based on requirements, documents, and graph info.
Achieves full automation without manual parameter setting.

Employs a step-by-step generation strategy:
1. Time configuration
2. Event configuration
3. Batch Agent configuration
...
class SimulationConfigGenerator:
    ...
    def _generate_time_config(self, context: str, num_entities: int) -> Dict[str, Any]:
        ...
        prompt = f"""Generate time simulation configuration based on the following requirements.
...
- People's daily routines (defaulting to standard 24h cycle)
- Late night (0-5 AM) has almost no activity (activity multiplier 0.05)
- Morning (6-8 AM) gradually becomes active (activity multiplier 0.4)
- Working hours (9 AM - 6 PM) moderately active (activity multiplier 0.7)
- Evening peak (7-10 PM) is the most active (activity multiplier 1.5)
- After 11 PM activity decreases (activity multiplier 0.5)
- **Important**: The values below are for reference. Adjust specific time slots based on the nature of the event and the characteristics of the participant group.
  - e.g., Students might peak at 9-11 PM; Media is active all day; Official agencies only during work hours.
  - e.g., Breaking news might cause late-night discussions, shortening off_peak_hours.
...
- agents_per_hour_min (int): Min active agents per hour (Range: 1-{max_agents_allowed}).
- agents_per_hour_max (int): Max active agents per hour (Range: 1-{max_agents_allowed}).
- peak_hours (int array): Peak time slots.
- off_peak_hours (int array): Low activity slots.
- morning_hours (int array): Morning slots.
- work_hours (int array): Work slots.
- reasoning (string): Brief explanation of why this configuration was chosen."""

        system_prompt = "You are a social media simulation expert. Return pure JSON format. Ensure time configuration matches realistic human activity patterns."
    ...
    def _generate_event_config(
        ...
        prompt = f"""Generate event configuration based on following requirements.
...
- Design initial posts, **each post must specify a poster_type**.

**IMPORTANT**: poster_type MUST be selected from the "Available Entity Types" above so that initial posts can be assigned to the appropriate Agents.
Example: Official statements should be posted by Official/University types, news by MediaOutlet, and student opinions by Student.
...
## Task
Generate an activity configuration for each entity. Please note:
- **Activity levels**: Higher for media/celebs, lower for official organizations.
- **Official Organizations** (University/GovernmentAgency): Low activity (0.1-0.3), active during work hours (9:00 AM - 5:00 PM), slow response (60-240 mins), high influence (2.5-3.0).
- **Media** (MediaOutlet): Medium activity (0.4-0.6), active throughout the day (7:00 AM - 11:00 PM), fast response (5-30 mins), high influence (2.0-2.5).
- **Individuals** (Student/Person/Alumni): High activity (0.6-0.9), mostly active in the evening (6:00 PM - 11:00 PM), fast response (1-15 mins), lower influence (0.8-1.2).
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configurations must reflect realistic human activity patterns."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Rule-based configuration generation for a single Agent"""
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            return {
                ...
                "active_hours": list(range(9, 18)),  # 9:00 AM - 5:59 PM
            ...
            return {
                ...
                "active_hours": list(range(7, 24)),  # 7:00 AM - 11:59 PM
            ...
            return {
                ...
                "active_hours": list(range(8, 22)),  # 8:00 AM - 9:59 PM
            ...
            # Students: Primarily active in evening, high frequency
            return {
                "activity_level": 0.8,
                "posts_per_hour": 0.6,
            ...
            # Alumni: Primarily active in evening
            return {
                "activity_level": 0.6,
                "posts_per_hour": 0.4,
