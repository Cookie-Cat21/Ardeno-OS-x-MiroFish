...
class ZepEntityReader:
    ...
    2. Filter nodes matching predefined entity types (labels other than 'Entity').
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Retrieve all nodes from graph (paginated)
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Retrieve all edges from graph (paginated)
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Retrieve all related edges for a specified node (with retry mechanism)
        ...
        try:
            edges = self._call_with_retry(
                ...
                operation_name=f"Retrieve node edges (node={node_uuid[:8]}...)"
            )
    ...
    def filter_defined_entities(
        ...
        Filtering Logic:
        - If a node's Labels only contains 'Entity', it doesn't match predefined types, so skip it.
        - If Labels contains tags other than 'Entity' and 'Node', it matches predefined types, so keep it.
        
        Args:
            graph_id: Graph ID
            defined_entity_types: Predefined entity type list (optional, filters specifically for these types)
            enrich_with_edges: Whether to retrieve associated edge info for each entity
            
        Returns:
            FilteredEntities: Filtered collection of entities
        """
    ...
    def get_entity_with_context(
        ...
        try:
            node = self._call_with_retry(
                ...
                operation_name=f"Retrieve node details (uuid={entity_uuid[:8]}...)"
            )
    ...
    def get_entities_by_type(
        ...
        Args:
            ...
            enrich_with_edges: Whether to retrieve associated edge info
