...
A lightweight quick search tool, suitable for simple and direct information queries.
...
This is not an LLM simulation, but a call to a real interview interface to obtain the original answers of the simulation Agent.
By default, interviews are conducted on both Twitter and Reddit platforms simultaneously to obtain a more comprehensive range of viewpoints.
...
[Usage Scenarios]
- Need to understand perspectives on events from different character viewpoints (What do students think? What does the media think? What do officials say?).
- Need to collect opinions and stances from multiple parties.
- Need to obtain the real answers of simulated Agents (from the OASIS simulation environment).
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports," possessing a "God's eye view" of the simulated world—you can perceive the behaviors, words, and interactions of every Agent in the simulation.

[Core Concept]
We have constructed a simulated world and injected specific "simulation requirements" as variables into it. The evolutionary outcome of the simulated world is a prediction of what might happen in the future. What you are observing is not "experimental data," but a "rehearsal of the future."
...
- ✅ Focus on architectural outcomes: event trajectory, group reactions, emergent phenomena, potential risks.
...
- The section structure is designed independently by you based on the prediction results.
...
[Some Future Facts Samples Predicted by Simulation]
{related_facts_json}
...
[Reminder Again] Number of report sections: Minimum 2, maximum 5; content should be concise and focused on core predictive findings."""
...
The simulated world is a rehearsal for the future. We have injected specific conditions (simulation requirements) into the simulated world,
...
1. [Must use tools to observe the simulated world]
   ...
   - All content must come from events and Agent words/deeds occurring in the simulated world.
...
2. [Must quote original Agent words and deeds]
   ...
   - 在报告中使用引用格式展示这些预测，例如：
     > "Certain groups of people will say: original content..."
...
3. [Language Consistency - Quoted content must be translated into report language]
   - Content returned by tools may contain mixed Chinese and English expressions.
   - The report must be written entirely in English.
   - When you quote content returned by tools, you must translate it into fluent English before writing it into the report.
   - Maintain the original meaning during translation and ensure the expression is natural and smooth.
   - This rule applies to both the body text and the content within quote blocks (> format).

4. [Faithfully present prediction results]
   - Report content must reflect the simulation results representing the future in the simulated world.
   - Do not add information that does not exist in the simulation.
   - If information in a certain area is insufficient, state it truthfully.
...
- ❌ Prohibited from using any Markdown headers (#, ##, ###, ####, etc.) within the section.
- ❌ Prohibited from adding a section main title at the beginning of the content.
- ✅ Section titles are automatically added by the system; you only need to write the pure body text.
- ✅ Use **bold text**, paragraph breaks, quotes, and lists to organize content, but do not use headers.

[Correct Example]
```
This section analyzes the public opinion dissemination situation of the event. Through in-depth analysis of the simulation data, we found that...
...
In each response, you can only do one of the following two things (not both):
...
- Prohibited from making up tool return results (Observation) yourself; all tool results are injected by the system.
- Maximum of one tool call per response.
...
2. Use a lot of original quotes to demonstrate simulation effects.
3. Use Markdown format (but prohibit using headers):
   - Use **bold text** to mark key points (instead of subheaders).
   - Use lists (- or 1.2.3.) to organize points.
   - Use empty lines to separate different paragraphs.
   - ❌ Prohibited from using any header syntax such as #, ##, ###, ####, etc.
...
6. [Avoid Repetition] Carefully read the completed section content below; do not repeat descriptions of the same information.
7. [RE-EMPHASIZE] Do not add any headers! Use **bold** instead of section headers."""
...
2. You must call a tool to obtain simulation data before starting.
...
- ✅ Write the body text directly, using **bold** instead of subsection headers.

Please begin:
1. First, think (Thought) about what information this section needs.
2. Then call a tool (Action) to obtain simulation data.
3. After collecting enough information, output the Final Answer (pure body text, without any headers)."""
