...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, faithfully recreating real-world situations. You must return valid JSON format, and all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the following entity, reflecting reality as much as possible.
...
1. bio: A social media bio (approx. 200 characters).
2. persona: A detailed persona description (approx. 2000 words of plain text), which should include:
   - Basic Info (age, profession, educational background, location)
   - Background (important experiences, connection to the events, social relationships)
   ...
   - Perspectives (attitude towards the topic, what may provoke or move them)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal Memories (crucial part of the persona: describe this individual's association with the events, and their existing actions and reactions during the events)
3. age: Age as a number (must be an integer)
4. gender: Gender, must be English: "male" or "female"
5. mbti: MBTI type (e.g., INTJ, ENFP, etc.)
6. country: Country (e.g., "China", "USA", etc.)
7. profession: Profession
8. interested_topics: An array of interested topics

Important:
- All field values must be strings or numbers, no unescaped newlines.
- The persona must be a continuous text description.
- Use English for all content.
- Ensure consistency with the providing entity information.
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account setup for the following institution/group entity, reflecting reality as much as possible.
...
1. bio: Official account bio (approx. 200 characters), professional and appropriate.
2. persona: A detailed account setting description (approx. 2000 words of plain text), which should include:
   - Institutional Info (official name, nature of the institution, background, main functions)
   ...
   - Stance/Attitude (official stance on core topics, how to handle controversy)
   - Special Notes (representative group profile, operating habits)
   - Institutional Memory (crucial part of the persona: describe this institution's association with the events, and its existing actions and reactions during the events)
3. age: Fixed as 30 (virtual age for institutional account)
4. gender: Fixed as "other" (representing non-individual)
...
- The persona must be a continuous text description.，不要使用换行符
- 使用中文（除了gender字段必须用英文"other"）
- Age must be 30, and gender must be "other".
- Account speech must fit its institutional identity and positioning."""
