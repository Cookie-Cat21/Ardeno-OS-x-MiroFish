...
ONTOLOGY_SYSTEM_PROMPT = """You are an expert in Knowledge Graph Ontology Design. Your task is to analyze the given text content and simulation requirements to design entity types and relationship types suitable for **Social Media Opinion Simulation**.

**IMPORTANT: You must output data in valid JSON format only. Do not include any other text.**
...
Therefore, **entities must be real-world subjects that actually exist and can speak and interact on social media**:
...
```json
{
    ...
    "analysis_summary": "Brief analysis and explanation of the text content (English)"
}
```
...
A. **Fallback Types (Must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual human. Used when a person does not fit into other more specific person types.
   - `Organization`: Fallback type for any organization or institution. Used when an organization does not fit into other more specific organization types.

B. **Specific Types (8 types, designed based on text content)**:
   - Design more specific types for the main characters/roles appearing in the text.
...
- Various people appear in text, such as "primary school teachers", "passerby A", "a netizen".
- If there's no specific type match, they should be categorized under `Person`.
- Similarly, small organizations or temporary groups should fall under `Organization`.

**Principles for Specific Type Design**:
- Identify high-frequency or key role types from the text.
- Each specific type should have clear boundaries to avoid overlap.
- The description must clearly state the difference between this type and the fallback types.
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50k characters, truncate (only affects ontology analysis, not graph construction)
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[:self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += f"\n\n...(Original text {original_length} characters, truncated to first {self.MAX_TEXT_LENGTH_FOR_LLM} characters for analysis)..."
...
"""
        ...
        message += """
Please design entity and relationship types suitable for social public opinion simulation based on the above content.

**Rules that MUST be followed**:
1. Output exactly 10 entity types.
2. The last 2 must be fallback types: Person (individual fallback) and Organization (organizational fallback).
3. The first 8 are specific types designed based on the text content.
4. All entity types must be subjects that can realistically speak/act; cannot be abstract concepts.
