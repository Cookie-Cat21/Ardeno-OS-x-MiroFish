...
Encapsulates tools for graph search, node reading, edge queries, etc., for use by the Report Agent.

Core retrieval tools (optimized):
1. InsightForge (Deep Insight Retrieval) - Powerful hybrid search, auto-generates sub-questions and multi-dimensional retrieval.
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            f"Analytic Question: {self.query}",
            f"Prediction Scenario: {self.simulation_requirement}",
            f"\n### Predictive Data Statistics",
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        """Convert to text format (full version, non-truncated)"""
        text_parts = [
            f"## Panorama Search Results (Future Panorama View)",
            f"Query: {self.query}",
            f"\n### Statistics",
            f"- Total Nodes: {self.total_nodes}",
            f"- Total Edies: {self.total_edges}",
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts.append("\n### Interview Summary & Core Viewpoints")
...
class ZepToolsService:
    ...
    - get_node_detail - Get detailed info for a node
