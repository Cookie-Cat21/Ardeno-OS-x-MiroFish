...
class AgentActivity:
    ...
    def _describe_repost(self) -> str:
        ...
        if original_content and original_author:
            ...
            return f"reposted a message: '{original_content}'"
        elif original_author:
            return f"reposted {original_author}'s post"
        return "reposted a message"
    
    def _describe_quote_post(self) -> str:
        """Quote a post - includes original content, author information, and quote comment"""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
        ...
        if original_content and original_author:
            ...
            base = f"quoted {original_author}'s message"
        else:
            base = "quoted a post"
    ...
    def _describe_follow(self) -> str:
        ...
        if target_user_name:
            return f"started following '{target_user_name}'"
        return "followed a user"
    
    def _describe_create_comment(self) -> str:
        """Post a comment - includes comment content and post information"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation action logs and updates new agent activities into the Zep graph in real-time.
    Activities are grouped by platform and sent in batches after accumulating BATCH_SIZE activities.
    ...
    # Send interval (seconds) to prevent flooding
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Platform-grouped activity buffers
        ...
        self._total_sent = 0        # Successful batches sent
        self._total_items_sent = 0  # Successful activities sent
        self._failed_count = 0      # Failed batches
        self._skipped_count = 0     # Skipped activities (DO_NOTHING)
