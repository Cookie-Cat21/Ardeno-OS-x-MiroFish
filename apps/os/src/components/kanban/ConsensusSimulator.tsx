        ...
        {steps.map(step => (
          <div key={step.id} className="flex items-center gap-3 text-sm">
            ...
            <div className="w-5 h-5 shrink-0 flex items-center justify-center">
              {step.status === 'done'   && <Check className="h-4 w-4 text-chart-2" />}
              {step.status === 'active' && <Loader2 className="h-4 w-4 text-primary animate-spin" />}
              {step.status === 'wait'   && <div className="h-2 w-2 rounded-full bg-border" />}
            </div>
            <span className={
              step.status === 'done'   ? 'text-chart-2'
        ...
        {result && (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="ardeno-panel rounded-lg p-3 text-center border border-primary/20">
                <p className="text-muted-foreground text-[10px] uppercase tracking-widest mb-1">Consensus</p>
                <p className="text-primary text-2xl font-bold font-display">{result.consensus}%</p>
              </div>
              <div className="ardeno-panel rounded-lg p-3 text-center border border-border">
                <p className="text-muted-foreground text-[10px] uppercase tracking-widest mb-1">Risk</p>
            ...
            <div className="flex gap-2 pt-1">
              <button
                ...
                disabled={running}
                className="flex-1 flex items-center justify-center gap-2 rounded-lg py-2 text-sm text-muted-foreground border border-border hover:border-primary/30 hover:text-foreground transition-all disabled:opacity-50"
              >
              ...
              <button
                onClick={applyResult}
                disabled={applied}
                className="flex-1 rounded-lg py-2 text-sm font-medium bg-primary text-background hover:bg-primary/80 transition-colors disabled:opacity-50"
              >
