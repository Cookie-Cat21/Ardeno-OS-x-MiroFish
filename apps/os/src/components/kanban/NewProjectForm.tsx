    ...
    >
      ...
      <AnimatePresence>
        <motion.div
          ...
          className="relative z-10 w-full max-w-lg ardeno-panel rounded-2xl border border-border shadow-xl overflow-hidden"
          onClick={e => e.stopPropagation()}
        >
          {/* Header */}
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <div>
              <h2 className="text-foreground font-semibold font-display">{t('newProject')}</h2>
              <p className="text-muted-foreground text-xs mt-0.5">Add to the pipeline</p>
            </div>
            <button
              onClick={() => onOpenChange(false)}
              className="rounded-lg p-1.5 text-muted-foreground hover:text-foreground hover:bg-card-3 transition-colors"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className={cn(
                    ...
                    errors.client_name ? 'border-red-400/60' : 'border-border'
                  )}
                />
                {errors.client_name && (
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className={cn(
                  'w-full rounded-lg bg-card-3 border text-sm text-foreground px-3 py-2 outline-none',
                  'placeholder:text-muted-foreground/50 focus:border-primary/50 transition-colors',
                  errors.title ? 'border-red-400/60' : 'border-border'
                )}
              />
              {errors.title && (
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  type="number"
                  placeholder="5000"
                  className="w-full rounded-lg bg-card-3 border border-border text-sm text-foreground px-3 py-2 outline-none placeholder:text-muted-foreground/50 focus:border-primary/50 transition-colors"
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    {...register('deadline')}
                    type="date"
                    className="w-full rounded-lg bg-card-3 border border-border text-sm text-foreground px-3 py-2 pr-8 outline-none focus:border-primary/50 transition-colors"
                  />
            ...
            {/* GitHub Branch */}
            <div className="space-y-1.5">
              <label className="text-muted-foreground text-xs">GitHub Branch</label>
              <input
