    ...
    <div className="flex flex-col w-56 shrink-0">
      ...
      >
        <SortableContext items={projects.map(p => p.id)} strategy={verticalListSortingStrategy}>
          ...
          {projects.length === 0 && (
            <div className={cn(
              'flex items-center justify-center h-16 rounded-lg border border-dashed text-xs text-muted-foreground transition-colors',
              isOver ? `border-primary/40 ${meta.color}` : 'border-border/40'
            )}>
