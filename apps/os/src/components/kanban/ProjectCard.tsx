  ...
  return (
    <>
      ...
      <div
        ...
        {...listeners}
        className={cn(
          'group ardeno-panel rounded-lg p-3.5 border cursor-grab active:cursor-grabbing',
          'transition-all duration-200 hover:border-primary/30 hover:-translate-y-0.5 hover:shadow-card-hover',
      ...
      >
        {/* Loading overlay */}
        {isLoading && (
          <div className="absolute inset-0 rounded-lg bg-background/70 flex items-center justify-center z-10 backdrop-blur-[2px]">
        ...
        <div className="flex items-center justify-between mb-2.5">
          ...
          >
            <div className="h-full w-full rounded-full bg-card flex items-center justify-center">
              <span className="text-[8px] font-bold text-foreground">{project.consensus_score}</span>
            </div>
          </div>
        </div>
        ...
        {/* Title */}
        <h4
          className="text-foreground text-sm font-semibold leading-snug mb-0.5 cursor-pointer hover:text-primary transition-colors"
          onClick={e => { e.stopPropagation(); setShowDetail(true); }}
        >
          {truncate(project.title, 32)}
        </h4>
        <p className="text-muted-foreground text-[11px] mb-2.5">{project.client_name}</p>
        ...
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <span className="text-muted-foreground text-[9px]">{project.risk_score}%</span>
        </div>
        ...
        {project.moodboard_url && (
          <div className="relative rounded overflow-hidden mb-2.5 h-14 border border-border/50 group/mb">
            <img
        ...
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            ...
            className="flex items-center gap-1 rounded px-1.5 py-1 text-[10px] text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors"
          >
          ...
          {showCanvaButton && (
            <button
              ...
              className="flex items-center gap-1 rounded px-1.5 py-1 text-[10px] text-muted-foreground hover:text-purple-400 hover:bg-purple-400/10 transition-colors"
            >
          ...
          <button
            ...
            className="flex items-center gap-1 rounded px-1.5 py-1 text-[10px] text-muted-foreground hover:text-chart-2 hover:bg-chart-2/10 transition-colors disabled:opacity-50"
          >
            {exporting
          ...
          <button
            onClick={e => { e.stopPropagation(); setShowDetail(true); }}
            className="ml-auto flex items-center rounded px-1 py-1 text-[10px] text-muted-foreground hover:text-foreground hover:bg-card-3 transition-colors"
          >
      ...
      {showSimulator && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-sm ardeno-panel rounded-2xl border border-border shadow-xl p-6"
            onClick={e => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-5">
              <div>
                <h3 className="text-foreground font-semibold font-display">Agent Review</h3>
                <p className="text-muted-foreground text-xs mt-0.5">{project.title}</p>
              </div>
